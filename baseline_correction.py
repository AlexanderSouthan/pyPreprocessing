...
def correct_baseline(raw_data, mode, smoothing=True, transform=False, **kwargs):
    """
    ...
    """
    return raw_data - generate_baseline(raw_data, mode, smoothing=smoothing, transform=transform, **kwargs)


def generate_baseline(raw_data, mode, smoothing=True, transform=False, **kwargs):
    """
    Calculates and returns baseline data on input datasets with different algorithms. 
    ...
    mode: str      
        ...
        'convex_hull', 'ALSS', 'iALSS', 'drPLS', 'SNIP'.
    smoothing: bool
    ...
    if mode == baseline_modes[0]: # convex_hull
        ...
        for pp in tqdm(np.arange(1, n_iter+1)):
            ...
            r2 = (np.roll(raw_data, -pp,axis=1)[:, pp:spectrum_points-pp] + np.roll(raw_data, pp,axis=1)[:, pp:spectrum_points-pp])/2
